% pericardial_small_train_large_test_IMPROVED.m
clear; clc; close all;
...
% Improvement: Plot the solid line FIRST
% Biaxial (P22) - Blue solid line
h2 = plot(biaxial_smooth_lambda, biaxial_smooth_stress, '-', ...
    'Color', [0 0 1], 'LineWidth', 2.5);

% Improvement: Plot the dash-dot line SECOND so it overlays the solid one
% Uniaxial (P11) - Dark magenta dash-dot line
h1 = plot(uniaxial_smooth_lambda, uniaxial_smooth_stress, '-.', ... % More distinct than '--'
    'Color', [0.5 0 0.5], 'LineWidth', 2.5);

% Improvement: Use "hollow" markers for a professional look
% Uniaxial (P11) - Purple diamond markers with white faces
h3 = plot(P11_exp_data(:,1), P11_exp_data(:,2), 'd', ...
    'Color', [0.5 0 0.5], 'MarkerFaceColor', 'w', ... % Hollow marker
    'MarkerSize', 10, 'LineWidth', 1.5);

% Biaxial (P22) - Black circle markers with white faces
h4 = plot(P22_exp_data(:,1), P22_exp_data(:,2), 'o', ...
    'Color', 'k', 'MarkerFaceColor', 'w', ... % Hollow marker
...
% --- Set Axis Limits to Create the "Zoomed-In" View ---
ylim([0, 5]); 
xlim([1, 2.5]); 
...
% print('C_train_small_strain_linear_zoomed_improved_plot', '-dpng', '-r300');