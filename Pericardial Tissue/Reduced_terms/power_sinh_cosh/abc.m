...
% --- ENHANCEMENT: Add the Uniaxial Deformation Icon ---
try
    icon_img = imread('icon_uniaxial.png');
    icon_pos = [0.78, 0.78, 0.15, 0.15]; % [left, bottom, width, height]
    ax_icon = axes('Position', icon_pos);
    imshow(icon_img);
    set(ax_icon, 'Visible', 'off');
catch
    warning('icon_uniaxial.png not found. Skipping icon placement.');
end

% Create Legend with Sorted Percentages
...
% --- ENHANCEMENT: Add the Biaxial Deformation Icon ---
try
    icon_img = imread('icon_biaxial.png');
    icon_pos = [0.78, 0.78, 0.15, 0.15]; % [left, bottom, width, height]
    ax_icon = axes('Position', icon_pos);
    imshow(icon_img);
    set(ax_icon, 'Visible', 'off');
catch
    warning('icon_biaxial.png not found. Skipping icon placement.');
end
